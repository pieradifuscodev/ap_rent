...
export default function VehicleForm({ initialData, onSubmit, onCancel, loading }: VehicleFormProps) {
  ...
  const updateTariffa = (stagione: keyof VeicoloTariffe, campo: 'giornaliero' | 'settimanale', valore: string) => {
    ...
    setFormData(prev => ({
      ...prev,
      tariffe: { ...prev.tariffe, [stagione]: { ...prev.tariffe[stagione], [campo]: numVal } }
    }));
  };
  ...
  return (
    <form 
      ...
      className="flex flex-col xl:flex-row gap-10 items-start w-full max-w-none"
    >
      <div className="flex-1 space-y-10 w-full">
        ...
        <Card title="1. Identificazione Mezzo">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <Input label="Tipo Mezzo" as="select" value={formData.tipo} onChange={e => setFormData({...formData, tipo: e.target.value as any})}>
        ...
        {/* 2. TARIFFE (SEMPRE VISIBILI) */}
        <Card title="2. Prezzi Noleggio (€)">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {(['bassa', 'media', 'alta'] as const).map(stagione => (
              <div key={stagione} className="space-y-6 p-8 bg-[#fcfcfc] rounded-[2.5rem] border border-slate-100/50">
                <div className="text-center font-black uppercase text-[10px] tracking-[0.3em] text-slate-400 mb-2">{stagione} stagione</div>
                <Input label="Giorno" type="number" step="0.01" className="text-center font-black !text-xl" value={formData.tariffe[stagione].giornaliero} onChange={e => updateTariffa(stagione, 'giornaliero', e.target.value)} />
                <Input label="Settimana" type="number" step="0.01" className="text-center font-black !text-xl" value={formData.tariffe[stagione].settimanale} onChange={e => updateTariffa(stagione, 'settimanale', e.target.value)} />
              </div>
            ))}
          </div>
        </Card>

        {/* 3. SCADENZE (NASCOSTE PER BICI) */}
        {!isBici && (
          <Card title="3. Scadenze & Tecnica">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <Input 
                label={isBici ? "Livello Batteria %" : "Livello Carburante"} 
                type="number" 
                value={isBici ? formData.capacita_carburante : formData.totale_tacche}
              onChange={e => setFormData({...formData, totale_tacche: parseInt(e.target.value) || 0})}
              />
              <Input label="KM Attuali" type="number" value={formData.km_attuali} onChange={e => setFormData({...formData, km_attuali: parseInt(e.target.value) || 0})} />
            </div>
          </Card>
        )}
        ...
        <Card title={isBici ? "3. Note" : "4. Note"}>
          <Input as="textarea" rows={4} value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} placeholder="Stato batteria, accessori inclusi, ecc..." />
        </Card>
      </div>
