...
export default function ClientiPage() {
  ...
  // FILTRO DOPPIO: Tab (Privato/Azienda) + Ricerca testuale
  const filteredClients = clients.filter(c => {
    const searchLower = searchQuery.toLowerCase();
    
    // Logica per le Tab: Azienda se ha P.IVA, altrimenti Privato
    const isAzienda = c.partita_iva && c.partita_iva.trim() !== "";
    const matchesTab = 
      filter === 'tutti' || 
      (filter === 'azienda' && isAzienda) || 
      (filter === 'privato' && !isAzienda);

    const matchesSearch = 
      c.nome.toLowerCase().includes(searchLower) ||
      c.cognome.toLowerCase().includes(searchLower) ||
      (c.codice_fiscale && c.codice_fiscale.toLowerCase().includes(searchLower)) ||
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-tighter text-slate-900 uppercase">
            Anagrafica <span className="text-blue-600 italic">Clienti</span>
          </h1>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Link href="/admin/clienti/nuovo" className="bg-blue-600 text-white flex items-center gap-3 shadow-xl shadow-blue-500/10 px-6 py-3.5 rounded-2xl hover:bg-blue-500 transition-all">
        ...
        {filteredClients.length > 0 ? (
          ...
          <div className="py-20 text-center bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest">
              Nessun cliente trovato in questa categoria
            </p>
          </div>
        )}
      </div>
